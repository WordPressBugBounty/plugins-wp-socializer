window.wpsr_helpers = {
    ...
    set_ft_state_cookie: function(parent, value){
        var name = '';
        if(parent.getAttribute('data-save-state') != 'yes'){
            return;
        }
        if(parent.classList.contains('wpsr-sharebar')){
            name = 'wpsr_sb_state';
        }
        if(parent.classList.contains('wpsr-follow-icons')){
            name = 'wpsr_fi_state';
        }
        this.set_cookie(name, value, 0);
    }
...
document.addEventListener( 'DOMContentLoaded', function(){
    ...
    if( the_sb ){
        ...
        the_sb.style.display = 'none';
        var sb_state = wpsr_helpers.get_cookie('wpsr_sb_state');
        if(sb_state == 'closed' && the_sb.getAttribute('data-save-state') == 'yes'){
            wpsr_helpers.addClass(the_sb, 'wpsr-cookie-closed');
            wpsr_helpers.addClass(the_sb, closed_class);
        }
            ...
            window.addEventListener(e, function(){
                for( var i = 0; i < wpsr.length; i++ ){
                    ...
                    if(current_action == 'close'){
                        ...
                        if(!wpsr_ele.classList.contains('wpsr-cookie-closed')){
        ...
        for( i = 0; i < close_btns.length; i++ ){
            ...
            close_btn.addEventListener( 'click', function(){
                ...
                if( parent.classList.contains( closed_class ) ){
                    ...
                    wpsr_helpers.set_ft_state_cookie(parent, 'opened');
                }else{
                    wpsr_helpers.addClass( parent, closed_class );
                    wpsr_helpers.set_ft_state_cookie(parent, 'closed');
                }
            });
...
function socializer_native_share(e){
    e.preventDefault();
    if (navigator.share) { 
        navigator.share({
